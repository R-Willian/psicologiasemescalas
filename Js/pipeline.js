...
(function () {
    ...
    function updateScroll() {
        ...
        const end = rect.bottom - (viewHeight * 0.7);
            ...
            if (entry.isIntersecting) {
                ...
                if (node) {
                    ...
                    node.classList.add(
                        'border-[#C8A97E]/60',
                        'bg-[#C8A97E]',
                        'shadow-[0_0_14px_rgba(200,169,126,0.6)]'
                    );
                }
                obs.unobserve(card);
            }
...
const barraNivel = document.getElementById('barra-nivel');
const statusTexto = document.getElementById('status-texto');

// Suas frases personalizadas
const mensagens = {
    0: "ALTO RISCO (OPERAÇÃO CRÍTICA)",
    1: "SISTEMA VULNERÁVEL (1/5 MEDIDAS)",
    2: "SISTEMA VULNERÁVEL (2/5 MEDIDAS)",
    3: "SISTEMA VULNERÁVEL (3/5 MEDIDAS)",
    4: "SISTEMA VULNERÁVEL (4/5 MEDIDAS)",
    5: "AUTORIZADO PARA VOO (COMPLIANCE TOTAL)"
};
...
function atualizarChecklist() {
    ...
    checkboxes.forEach(box => {
        // Encontra o cartão que o usuário clicou
        const cartao = box.closest('.check-item-card');

        // Verifica se está marcado e acende/apaga o cartão
        if (box.checked) {
            selecionados++;
    ...
    // 1. Atualiza a largura da barra (Cada item vale 20%)
    const porcentagem = selecionados * 20;
    barraNivel.style.width = `${porcentagem}%`;

    // 2. Atualiza a frase correspondente
    statusTexto.textContent = mensagens[selecionados];

    // 3. Atualiza a cor (O CSS cuida do resto)
    statusTexto.className = 'status-mensagem nivel-' + selecionados;
}

// Cria o ouvinte para disparar a função toda vez que houver um clique
checkboxes.forEach(box => {
    box.addEventListener('change', atualizarChecklist);
});
